...
class PsCo(MoCo):
    ...
    def cross_domain_adaptation(self, inputs, device):
        self.eval()
        ...
        q = F.normalize(self.predictor(self.projector(inputs)))
        k = F.normalize(self.projector(inputs))
        
        # moco loss
        logits_moco = torch.einsum('ad, bd -> ab', q, k)
        
        labels_moco = torch.arange(0, batch_size, 1).long().to(device)
        loss_moco = F.cross_entropy(logits_moco / 0.2, labels_moco)
        
        return loss_moco
    
    def fewshot_acc(self, args, inputs, labels, num_ways, adaptation, optimizer, device):
        with torch.no_grad():
        ...
        for x_support, x_query, y_support, y_query in tasks:
            if adaptation:
                loss = self.cross_domain_adaptation(x_support, device)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            self.eval()
            with torch.no_grad():
                x_support = F.normalize(self.projector(x_support))
                x_query = F.normalize(self.predictor(self.projector(x_query))) # q d
